...
def sheet_boss_stats():
    """Запрашиваем агрегированную статистику для /boss у Apps Script."""
    if not SHEET_SCRIPT_URL or not SHEET_API_KEY:
        print("SHEET: missing SHEET_SCRIPT_URL/SHEET_API_KEY")
        return {"ok": False, "error": "config"}
    try:
        r = requests.post(
            SHEET_SCRIPT_URL,
            headers={"Authorization": f"Bearer {SHEET_API_KEY}"},
            json={"action": "boss_stats"},
            timeout=12,
        )
        print("sheet_boss_stats:", r.status_code, r.text[:200])
        return (
            r.json()
            if r.headers.get("content-type", "").startswith("application/json")
            else {"ok": r.ok}
        ...
        print("sheet_boss_stats error:", e)
        return {"ok": False, "error": str(e)}
...
def make_message(data: Dict[str, Any]) -> str:
    ...
    if get("invoice"):
        lines.append(f"📄 <b>Счёт/КП:</b> {get('invoice')}")
    return "\n".join(lines)
...
async def tg_webhook(req: Request):
    ...
    # --- текст от пользователя (для «НА ДОРАБОТКУ» и команд) ---
    if "message" in upd:
        msg  = upd["message"]
        ...
        if text.startswith("/boss"):
            stats = sheet_boss_stats()
            if not stats.get("ok"):
                err = stats.get("error", "unknown")
                tg_send_message(f"⚠️ Не удалось получить статистику: {html.escape(str(err))}")
                return {"ok": True}

            urgent      = stats.get("urgent", 0)
            priority    = stats.get("priority", 0)
            revise_week = stats.get("revise_week", 0)
            per_exec    = stats.get("per_executor", {})

            lines = [
                "📊 <b>Панель /boss</b>",
                f"🚨 Аварийные заявки: <b>{urgent}</b>",
                f"⭐ Приоритетные заявки: <b>{priority}</b>",
            ]

            if per_exec:
                lines.append("\n👷 <b>Активные заявки по исполнителям:</b>")
                for name, cnt in per_exec.items():
                    lines.append(f"• {html.escape(str(name))}: <b>{cnt}</b>")
            else:
                lines.append("\n👷 Активных заявок по исполнителям нет")

            lines.append(f"\n🛠 <b>На доработку за 7 дней:</b> {revise_week}")
