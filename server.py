...
app = FastAPI(title="SnabOrders Bot", version="2.2")
...
async def tg_webhook(req: Request):
    ...
    if "callback_query" in upd:
        ...
        if action == "received":
            ...
            sheet_update_status(order_id, new_status)
            tg_send_message(f"📦 <b>ТМЦ ПОЛУЧЕНО</b> по заявке <b>{order_id}</b>.\nНажал: {who}")

        elif action == "approve":
            new_status = "В РАБОТУ: СОГЛАСОВАНО"
            sheet_update_status(order_id, new_status)
            tg_send_message(f"✅ <b>СОГЛАСОВАНО</b> по заявке <b>{order_id}</b>.\nНажал: {who}")
    ...
    if "message" in upd:
        ...
        if uid in PENDING_REVISE and text:
            ...
            sheet_update_status(order_id, new_status, comment=text)
            tg_send_message(f"🔧 <b>На доработку</b> по заявке <b>{order_id}</b>.\nКомментарий: {text}")
        return {"ok": True}
